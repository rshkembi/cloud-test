...
use Aheadworks\Popup\Controller\Ajax\AddCookie;
...
class DataProvider implements ArgumentInterface
{
    ...
    private function getPopupsArrayContentHtml($blockType)
    {
        ...
        if ($this->request->getParam('preview', 0)) {
            ...
            $popupsContentArr[$id]['content'] =
                ...
                '<style type="text/css">' . $popupInfo->getData('customCss') . '</style></div>';
    ...
    {
        return $this->customerSession->getCookiePath();
    }
    ...
    {
        return $this->customerSession->getCookieDomain();
    }

    /**
     * Get default cookie life time
     *
     * @return int
     */
    public static function getDefaultCookieLifetime(): int
    {
        return AddCookie::DEFAULT_COOKIE_LIFETIME;
    }
}
