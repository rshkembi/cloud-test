define([
    ...
    $.widget('mage.blogCategoryTree', {
         ...
         * @private
         */
        _formatCategories: function () {
            var i;
            for(i = 0 ; i < this.options.categories.length ; i++) {
                this.options.categories[i]['text'] =  this._escapeHtml( this.options.categories[i]['text']);
            }
        },
    ...
    /**
        ...
        _bind: function () {
            ...
            if (tree.length) {
                var self = this;
                tree.on('move_node.jstree3', $.proxy(this._moveNode, this));
                tree.on('select_node.jstree3', function (event, node) {
                    var selected = node.instance.get_selected();
                    self.element.parents('.chooser_container').find('.input-text.entities').val(selected.join());
                });

                tree.on('deselect_node.jstree3', function (event, node) {
                    var selected = node.instance.get_selected();
                    self.element.parents('.chooser_container').find('.input-text.entities').val(selected.join());
                });
